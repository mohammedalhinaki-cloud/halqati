              ...
              {memSelected.length>0 && (
                <div className="mt-2 space-y-2">
                  ...
                  {memSelected.map(sel=>{
                    ...
                    return (
                      <div key={sel.surahNumber} className="bg-[#FAF9F4] rounded-xl border p-2 w-full max-w-full box-border overflow-hidden">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-1.5 w-full max-w-full box-border">
                          <div className="min-w-0"><label className="text-[11px] font-bold">من آية</label><div className="flex items-center gap-1 mt-1 w-full max-w-full box-border min-w-0"><button type="button" onClick={()=> setMemSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, fromAyah: Math.max(1, x.fromAyah-1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">−</button><input type="number" value={sel.fromAyah} onChange={e=> setMemSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, fromAyah: Number(e.target.value)||1} : x))} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg border text-center text-sm font-bold box-border" /><button type="button" onClick={()=> setMemSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, fromAyah: Math.min(surah.ayahCount, x.fromAyah+1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">+</button></div></div>
                          <div className="min-w-0"><label className="text-[11px] font-bold">إلى آية</label><div className="flex items-center gap-1 mt-1 w-full max-w-full box-border min-w-0"><button type="button" onClick={()=> setMemSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, toAyah: Math.max(x.fromAyah, x.toAyah-1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">−</button><input type="number" value={sel.toAyah} onChange={e=> setMemSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, toAyah: Number(e.target.value)||1} : x))} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg border text-center text-sm font-bold box-border" /><button type="button" onClick={()=> setMemSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, toAyah: Math.min(surah.ayahCount, x.toAyah+1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">+</button></div></div>
                        </div>
                      </div>
                    )
                  })}
                  ...
                  {reviewSelected.map(sel=>{
                    ...
                    return (
                      <div key={sel.surahNumber} className="bg-[#FAF9F4] rounded-xl border p-2 w-full max-w-full box-border overflow-hidden">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-1.5 w-full max-w-full box-border">
                          <div className="min-w-0"><label className="text-[11px] font-bold">من آية</label><div className="flex items-center gap-1 mt-1 w-full max-w-full box-border min-w-0"><button type="button" onClick={()=> setReviewSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, fromAyah: Math.max(1, x.fromAyah-1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">−</button><input type="number" value={sel.fromAyah} onChange={e=> setReviewSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, fromAyah: Number(e.target.value)||1} : x))} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg border text-center text-sm font-bold box-border" /><button type="button" onClick={()=> setReviewSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, fromAyah: Math.min(surah.ayahCount, x.fromAyah+1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">+</button></div></div>
                          <div className="min-w-0"><label className="text-[11px] font-bold">إلى آية</label><div className="flex items-center gap-1 mt-1 w-full max-w-full box-border min-w-0"><button type="button" onClick={()=> setReviewSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, toAyah: Math.max(x.fromAyah, x.toAyah-1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">−</button><input type="number" value={sel.toAyah} onChange={e=> setReviewSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, toAyah: Number(e.target.value)||1} : x))} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg border text-center text-sm font-bold box-border" /><button type="button" onClick={()=> setReviewSelected(prev=> prev.map(x=> x.surahNumber===sel.surahNumber ? {...x, toAyah: Math.min(surah.ayahCount, x.toAyah+1)} : x))} className="w-7 h-7 shrink-0 rounded-lg bg-white border font-bold">+</button></div></div>
                        </div>
                      </div>
                    )
                  })}
